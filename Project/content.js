...
{
    loopCount++;
    if (loopCount > 15) { // After 15 seconds give up looking
        console.log("Timed out while looking for video")
        loopCount = 0;
        clearInterval(checkExist);
    }
    var inside = document.getElementsByClassName("embedded-player-container show")
    var interior = inside[0]
...
function attachChat(enabled) // Function to embed the div and iframe that contain the twitch chat
{
    ...
    var interior = document.getElementsByClassName("embedded-player-container show")[0]
    ...
    iframe.src = "https://twitch.tv/embed/livechatf1/chat?parent=f1tv.formula1.com"
...
function attachButton()
{
    ...
    var list = document.getElementsByClassName("channel-switcher-controls-list list-unstyled mb-0 inline")[0]
    ...
    var all = document.getElementsByClassName("channel-switcher-container switcher-hidden")[0]
    
    if (all != null) {
        all.style.left = 0;
        all.style.right = null;
    }
    ...
    button.type = "button"
    button.className  = "btn btn-link d-flex align-items-center live-race-button active"
    button.innerText = "Chat"
    button.addEventListener("click", switchChat);

    // var image = document.createElement("img")
    // image.src = chrome.runtime.getURL("images/F1Car.png")

    // button.appendChild(image)

    var description = document.createElement("span")
    description.className  = "switcher-description"
    description.innerText = "Toggle Twitch Chat"
